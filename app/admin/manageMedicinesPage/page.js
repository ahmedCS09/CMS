    ...
    if (user?.role !== 'admin') {
        return (
            <div className="min-h-screen bg-slate-950 pt-32 px-6 flex items-center justify-center">
                <div className="text-center bg-slate-900 p-12 rounded-[2.5rem] shadow-xl shadow-black/40 border border-slate-800 max-w-md">
                    ...
                    <p className="text-slate-500 mt-2 mb-8 font-medium uppercase tracking-tight">This terminal is reserved for administrators.</p>
                    <button onClick={() => router.push('/')} className="px-8 py-3 bg-slate-950 border border-slate-800 text-white rounded-xl font-bold hover:bg-slate-800 transition-all shadow-lg shadow-black/40">Go Home</button>
                </div>
            </div>
        )
    }
