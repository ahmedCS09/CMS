...
export default function Home() {
  return (
    <main suppressHydrationWarning className="min-h-screen bg-slate-900">
      {/* Hero Section */}
      <section className="relative pt-32 pb-20 px-6 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-16">
          <div className="lg:w-1/2 space-y-8 animate-in slide-in-from-left duration-700">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-500/10 border border-indigo-500/20 rounded-full text-indigo-600 text-sm font-black uppercase tracking-widest shadow-sm shadow-black/20">
            ...
            <div className="flex items-center gap-4">
              <Link
                href="/auth/registerPage"
                 className="group flex items-center gap-3 px-8 py-5 bg-indigo-600 text-white rounded-2xl font-black text-lg transition-all hover:bg-indigo-700 hover:scale-[1.02] active:scale-[0.98] shadow-2xl shadow-indigo-600/20"
              >
                Get Started free
              ...
              <Link
                href="/auth/loginPage"
                className="px-8 py-5 text-white font-black text-lg hover:text-indigo-600 transition-colors"
              >
                Sign in
              </Link>
            </div>
            <div className="flex items-center gap-8 pt-6 border-t border-slate-800">
              <div className="flex -space-x-3">
                {[1, 2, 3, 4].map((i) => (
                  <div key={i} suppressHydrationWarning className="w-10 h-10 rounded-full border-2 border-slate-800 bg-slate-900 overflow-hidden ring-1 ring-slate-800">
                    <img src={`https://i.pravatar.cc/100?img=${i + 15}`} alt="User" />
                  </div>
                ))}
              </div>
              <p className="text-sm font-bold text-slate-500">
                Trusted by <span className="text-white font-black tracking-tight">5,000+</span> healthcare practitioners
              </p>
            </div>
          </div>
          ...
          <div className="lg:w-1/2 relative group animate-in zoom-in duration-700">
            ...
            <div className="bg-slate-950 rounded-[2.5rem] p-4 border border-slate-800 shadow-2xl shadow-black/60">
              <img
            ...
            {/* Floating elements */}
            <div className="absolute top-1/2 -left-12 -translate-y-1/2 bg-slate-900 p-6 rounded-2xl shadow-xl shadow-black/40 border border-slate-800 animate-bounce-slow hidden md:block">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-green-500/10 rounded-full flex items-center justify-center text-green-400">
                  <CheckCircle2 size={24} />
                </div>
                <div className="space-y-1">
                  <p className="text-xs font-black uppercase text-slate-500">Success</p>
                  <p className="text-sm font-black text-white">Patient Recorded</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="py-24 bg-slate-950 relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center max-w-3xl mx-auto mb-20 space-y-6">
            <h2 className="text-4xl font-black text-white tracking-tighter">Everything you need to <span className="text-indigo-600 underline underline-offset-8 decoration-indigo-200">run your clinic</span>.</h2>
      ...
      <section className="py-24 px-6">
        <div className="max-w-7xl mx-auto bg-slate-900 rounded-[3rem] p-12 lg:p-20 relative overflow-hidden flex flex-col items-center text-center gap-10 border border-slate-800 shadow-2xl shadow-black/80">
          <div className="space-y-4 relative z-10">
            <h2 className="text-4xl lg:text-5xl font-black text-white tracking-tighter">Join the future of healthcare.</h2>
            <p className="text-xl text-slate-500 font-bold max-w-lg mx-auto leading-relaxed">Start managing your clinic with MediConnect today and experience the difference in efficiency.</p>
          </div>
          <div className="flex flex-col sm:flex-row gap-4 relative z-10 w-full max-w-md">
            ...
            />
            <button className="px-8 py-5 bg-indigo-600 text-white rounded-2xl font-black text-lg hover:bg-indigo-700 transition-all active:scale-95 shadow-xl shadow-black/40 shadow-indigo-900/40">
              Join Now
            </button>
          </div>
...
const FeatureCard = ({ icon, title, desc }) => (
  <div className="group p-8 bg-slate-900 border border-slate-800 rounded-[2rem] hover:ring-4 hover:ring-indigo-600/5 hover:border-indigo-600/20 transition-all duration-300">
